...
try:
    ...
    sys.path.append(x)
    from ge.models import Connector, Logs, Term, WFControl, WordMap  # noqa E402
except:  # noqa E722
    raise
...
def map(connector="all", chunck=1000000, schema=1) -> bool:
    ...
    if schema in [1, 2]:
        ...
        # Log-Message
        v_desc = "Only schemes 1 and 2 available"
        print(v_desc)
    ...
    if DF_KEY.empty:
        # Log-Message
        v_desc = "The Term table has no records"
        print(v_desc)
        Logs.objects.create(process=v_proc, description=v_desc, status="w")
        if v_schema == 2:
            # Log-Message
            v_desc = "It will not be possible to perform MapReduce without data in the Term table with schema 2. Register new Term or change to schema 1 in which all the words will save on WORDMAP"  # noqa E501
            print(v_desc)
            Logs.objects.create(process=v_proc, description=v_desc, status="e")
            # Stops this process and return False
            return False
    ...
    for qs in qs_queryset:
        # Log-Message
        v_desc = f"{qs.connector}: Start MAP Process"
        print(v_desc)
        Logs.objects.create(process=v_proc, description=v_desc, status="s")
        ...
        if not os.path.exists(v_target):
            # Log-Message
            v_desc = f"{qs.connector}: No Prepare file on {v_target}."
            print(v_desc)
            Logs.objects.create(process=v_proc, description=v_desc, status="w")
            # Stops this process and starts the next connector
            continue
        ...
        if v_schema == 2:
            # Log-Message
            v_desc = f"{qs.connector}: Option to eliminate words with no Term relationship is active (schema = 2)."  # noqa E501
            print(v_desc)
            Logs.objects.create(process=v_proc, description=v_desc, status="w")
        ...
        for fp in pd.read_csv(
            ...
            if v_idx == 1:
                # Log-Message
                v_desc = f"{qs.connector}: Start mapper on {v_chunk} rows per block."  # noqa E501
                print(v_desc)
                Logs.objects.create(
            ...
            try:
                ...
                # Log-Message
                v_desc = f"{qs.connector}: Check for data in the file generated by the Prepare Process. Error {e}"  # noqa E501
                print(v_desc)
                Logs.objects.create(
            ...
            # Log-Message
            v_desc = f"{qs.connector}: Block {v_idx} with {v_row} combinations processed."  # noqa E501
            print(v_desc)
            Logs.objects.create(
                process=v_proc, description=v_desc, status="s"
            )  # noqa E501
        ...
        qs_wfc.save()
        # Log-Message
        v_desc = f"{qs.connector}: Map process was completed successfully in {v_time} seconds with {v_count} records"  # noqa E501
        print(v_desc)
        Logs.objects.create(process=v_proc, description=v_desc, status="s")
    ...
    v_desc = f"MAP process was completed in {v_time} seconds"  # noqa E501
    print(v_desc)
    Logs.objects.create(process=v_proc, description=v_desc, status="s")
