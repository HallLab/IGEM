"""
MONDO Disease Relationship DTP.

Pipeline role:
- Runs AFTER dtp_disease_mondo (master).
- Reads relationship_data.parquet produced by dtp_disease_mondo transform.
- No extract or transform steps — those belong to dtp_disease_mondo.

What is loaded:
- EntityRelationship rows for Disease→Disease edges:
    is_a   → child disease is a type of parent disease
    part_of → child disease is part of broader parent category

  Both endpoints must already exist as Disease entities. Edges referencing
  terms not yet loaded are silently skipped; re-running after a full load
  will pick them up automatically.

Note:
  Gene→Disease and Chemical→Disease relationships come from CTD DTPs,
  not from MONDO.
"""
...
_PARENT_SOURCE = "disease_mondo"


class DTP(DTPBase, EntityQueryMixin):

    DTP_NAME = "dtp_disease_mondo_relationships"
    DTP_VERSION = "1.0.0"
    ...
    def extract(
        ...
        msg = (
            ...
            "raw data is owned by dtp_disease_mondo."
        )
    ...
    def transform(
        ...
        msg = (
            ...
            "relationship_data.parquet is generated by dtp_disease_mondo."
        )
        self.logger.log(msg, "INFO")
    ...
    def load(self, processed_dir: str) -> tuple[bool, str, ETLStepStats]:
        ...
        if not rel_file.exists():
            return (
                ...
                "run dtp_disease_mondo (extract + transform) first.",
        ...
        if disease_type_id is None:
            return (
                False, "EntityType 'Diseases' not found.", ETLStepStats(errors=1)
            )

        # Pre-load source (disease) entities: MONDO ID → entity_id
        ...
        # Write skipped rows to file for inspection
        if skipped_rows:
