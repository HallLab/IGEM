"""
Reactome Pathway Relationship DTP.

Pipeline role:
- Runs AFTER dtp_pathway_reactome (master), dtp_gene_hgnc/ncbi/ensembl (Genes),
  and dtp_protein_uniprot (Proteins).
- Reads relationship_data.parquet produced by dtp_pathway_reactome transform.
- No extract or transform steps — those belong to dtp_pathway_reactome.

What is loaded:
- EntityRelationship rows connecting pathways to:
    Pathways  → relation_type "part_of"    (R-HSA child → R-HSA parent)
    Genes     → relation_type "in_pathway" (gene symbol or ENSG ID → pathway)
    Proteins  → relation_type "in_pathway" (UniProt accession or ENSP ID → pathway)

  Target entities not yet loaded are silently skipped; re-running after loading
  the missing master data will pick them up automatically.
"""
...
_PARENT_SOURCE = "pathway_reactome"


class DTP(DTPBase, EntityQueryMixin):

    DTP_NAME = "dtp_pathway_reactome_relationships"
    DTP_VERSION = "1.0.0"
    ...
    def extract(
        ...
        msg = (
            ...
            "raw data is owned by dtp_pathway_reactome."
        )
    ...
    def transform(
        ...
        msg = (
            ...
            "relationship_data.parquet is generated by dtp_pathway_reactome."
        )
        self.logger.log(msg, "INFO")
    ...
    def load(self, processed_dir: str) -> tuple[bool, str, ETLStepStats]:
        ...
        if not rel_file.exists():
            return (
                ...
                "run dtp_pathway_reactome (extract + transform) first.",
        ...
        pathway_type_id = type_id_map.get("Pathways")
        if pathway_type_id is None:
            return (
                False,
                "EntityType 'Pathways' not found.",
                ETLStepStats(errors=1),
            )

        # Pre-load source (pathway) entities: Reactome ID → entity_id
        ...
        def _resolve_target(target_type: str, target_id: str) -> Optional[int]:
