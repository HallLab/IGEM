...
class TestAnnotateBasics:
    def test_merges_default_annotation_columns(self, fake_reports):
        res = _result_with(["rs00001", "TP53"])
        out = res.annotate(fake_reports)
    ...
    def test_unmatched_input_gets_nan(self, fake_reports):
    ...
    def test_keep_columns_restricts_join(self, fake_reports):
        res = _result_with(["rs00001"])
        out = res.annotate(fake_reports, keep_columns=["gene_symbol"])
        assert "gene_symbol" in out.df.columns
        assert "hgnc_id" not in out.df.columns

    def test_assembly_passed_through(self, fake_reports):
        res = _result_with(["rs00001"])
        res.annotate(fake_reports, assembly="GRCh38.p14")
        assert fake_reports.last_call["assembly"] == "GRCh38.p14"

    def test_only_unique_inputs_sent_to_server(self, fake_reports):
        res = _result_with(["rs00001", "rs00001", "TP53"])
        res.annotate(fake_reports)
        assert sorted(fake_reports.last_call["input_values"]) == [
            "TP53", "rs00001",
        ]


class TestAnnotateClientShapes:
    def test_accepts_object_with_reports_attr(self, fake_reports):
        # Simulate the IGEM facade exposing .reports.
        class FakeIGEM:
            def __init__(self, r):
                self.reports = r

        res = _result_with(["TP53"])
        out = res.annotate(FakeIGEM(fake_reports))
        assert "gene_symbol" in out.df.columns
    ...
    def test_unknown_input_col_raises(self, fake_reports):
        res = _result_with(["TP53"])
        with pytest.raises(ValueError, match="not_a_col"):
            res.annotate(fake_reports, input_col="not_a_col")


class TestAnnotateMetadata:
    def test_metadata_records_annotation(self, fake_reports):
        res = _result_with(["TP53"])
        out = res.annotate(fake_reports)
...
class TestAnnotateChainability:
    def test_chains_with_correction_and_top(self, fake_reports):
        res = _result_with(["rs00001", "TP53", "rs00002", "rs00003"])
        chained = (
            res
            .with_correction("bonferroni")
            .annotate(fake_reports)
            .top(2)
        )
