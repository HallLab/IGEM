...
def collect_teste():
    print("dentro do collect teste")
    print("-"*50)
    x = Connector.objects.all().first()
    return x


def collect(datasource='all', connector='all'):
    ...
    for qs in qs_queryset:
        ...
        try:
            ...
            print("    Could not find the version of the file. Check content-length attr")  # noqa E501
        ...
        if qs_wfc.source_file_version == v_version:
            print('    Version already loaded in: {0}'.format(str(qs_wfc.last_update)[0:19]))  # noqa E501   
            continue
        ...
        # New file version, start download
            ...
            if not os.path.exists(v_target_file):
                ...
                print('       Possible cause: check if the names of the source and destination files are correct in the connector table')  # noqa E501
