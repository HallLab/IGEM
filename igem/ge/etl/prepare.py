...
def mapper(lines):
    ...
    for idx, line in lines.iterrows():
        try:
            ...
            DF_KY_WD_TEMP = DF_KY_WD[DF_KY_WD['word'].str.contains(r'\b(?:\s|^)(?:{})(?:\s|$\b)'.format('|'.join(v_str)))] # noqa E501
...
def prepare(datasource='all', connector='all', chunk=1000000):
    ...
    for qs in qs_queryset:
        print('  Start: Run datasource {0} on Connector {1}'.format(qs.datasource, qs.connector)) # noqa E501
        v_time_ds = time.time()
        # Check Workflow
        try:
            qs_wfc = WFControl.objects.get(connector_id=qs.id, chk_collect=True, chk_prepare=False)  # noqa E501
        except ObjectDoesNotExist:
        ...
        try:
            if v_skip >= 1:
                v_read_file = {'sep': v_tab, 'skiprows': v_skip, 'engine': 'python', 'chunksize': v_chunk} # noqa E501
                print('    Open file with {0} skipped rows and {1} rows per block process'.format(v_skip, v_chunk)) # noqa E501
            else:
                v_read_file = {'sep': v_tab, 'engine': 'python', 'chunksize': v_chunk} # noqa E501
                print('    Open file without skips rows and {0} rows per block process'.format(v_chunk)) # noqa E501
            ...
            for df_source in pd.read_csv(v_source, **v_read_file):
                ...
                for n in range(v_col):  # Read transformations columns
                    ...
                    try:
                        try:
                            qs_col = DSTColumn.objects.get(connector_id=qs.id, column_number=n)  # noqa E501
                            v_col_idx = str(qs_col.column_name)
                        except ObjectDoesNotExist:
                            qs_col = None
                    except Exception as e:
                        print(e)  # add if to handle
                        print('    Error reading DST table to %s. Check Connector settings and ID duplicity.' % qs.connector)  # noqa E501

                    if not qs_col:
                        # Rule 1: Columns not configured on Connector master data. The process will run and perform the Commute # noqa E501
                        if v_idx == 1:
                            print('    No rules defines to column: %s. This column will consider on process.' % n) # noqa E501
                            print('      If necessary, correct the column through the Connector settings') # noqa E501
                        ...
                        df_target[v_col_idx] = df_target.set_index(v_col_idx).index.map(DF_KY_WD.set_index("word")["term_id__term"])  # Commute Process # noqa E501
                    else:
                        if qs_col.status:
                            if str(qs_col.pre_value) != 'none':
                                # Rule 2: columns with defined prefixes / does not perform the commute process  # noqa E501
                                df_target[qs_col.column_name] = df_source.iloc[:, n].apply(lambda y: "{}{}".format(qs_col.pre_value, y))# noqa E501
                                df_target = df_target.apply(lambda x: x.astype(str).str.lower())  # Keep all words lower case to match # noqa E501
                                continue
                            ...
                            if qs_col.single_word:
                                df_target[qs_col.column_name] = df_source.iloc[:, n] # noqa E501
                                df_target = df_target.apply(lambda x: x.astype(str).str.lower())  # Keep all words lower case to match # noqa E501
                                df_target[v_col_idx] = df_target.set_index(v_col_idx).index.map(DF_KY_WD.set_index("word")["term_id__term"])  # Commute Process # noqa E501
                                continue
                            ...
                            df_reducer = pd.DataFrame(columns=[qs_col.column_name]) # noqa E501
                            df_temp[qs_col.column_name] = df_source.iloc[:, n]
                            df_temp = df_temp.apply(lambda x: x.astype(str).str.lower()) # Keep all words lower case to match  # noqa E501
                            list_df = np.array_split(df_temp, os.cpu_count()-1)

                            try:
                                with ThreadPoolExecutor() as executor:
                                    future = {executor.submit(mapper, list_df[i]) for i in range(len(list_df))} # noqa E501
                                ...
                                print(e)
                                print('      Error on commute word by word on sentence') # noqa E501
                ...
                df_target.to_csv(v_target, header=header, mode='a') # Write the file# noqa E501
                header = False  # Prevent creating new header lines
                print('      Block {0} with {1} records processed'.format(v_idx, v_row))# noqa E501
            ...
            continue
        print('    Connector loaded in {0} seconds'.format(int(time.time() - v_time_ds)))    # noqa E501       

    print('End of process in {0} seconds'.format(int(time.time() - v_time_process)))  # noqa E501
