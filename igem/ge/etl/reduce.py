...
def reduce(connector='all', chunck=1000000):
    ...
    for qs in qs_queryset:
        ...
        DFR = pd.DataFrame(WordMap.objects.values("connector_id", "term_1_id", "term_2_id").filter(  # noqa E501
            connector_id=qs.id,
            term_1_id__isnull=False,
            term_2_id__isnull=False
            ).annotate(qtd_links=Sum("qtd_links")), columns=["connector_id", "term_1_id", "term_2_id", "qtd_links"]) # noqa E501
        ...
        print('    {0} records loaded from RoadMap will be aggregated'.format(v_size)) # noqa E501
        ...
        if not DFR.empty:
            ...
            while v_upper <= (v_size+v_chunk):
                ...
                model_TermMap = [TermMap(
                    ckey=str(str(record.connector_id) + '-' + str(record.Index)), # noqa E501
            ...
            print('    No data from {0} to update TermMap table'.format(qs.connector)) # noqa E501
    ...
    print('End of process in {0} seconds'.format(int(time.time() - v_time_proces))) # noqa E501
