...
def search_terms(lines):
    ...
    df_mapper = pd.DataFrame(columns=[
        'row',
        'string',
        'word',
        'term',
    ...
    for idx, line in lines.iterrows():
        ...
        v_line = line[0]            # v_line used to fix the String Column on output file # noqa E501
        v_row = line.name           # Keep the source number row to used in Row Column on output file # noqa E501
        ...
        try:
            ...
            if v_msm:
                print("--> Number of records found that match the list of searched elements: ", len(DF_KY_WD_TEMP.index)) # noqa E501
            ...
            for index, row in DF_KY_WD_TEMP.iterrows():
                ...
                if line[0].find(row['word']) != -1:
                    if v_msm:
                        print("  --> First loop, interaction number: ", v_seq, " found: ", row['word']) # noqa E501
                    ...
                    if re.search(r'\b' + row['word'] + r'\b', line[0]):
                        ...
                        if v_msm:
                            ...
                            print("    --> Word or Words Associated: ", row['word']) # noqa E501
                            print("    --> String after association: ", line[0]) # noqa E501
...
def word_to_term(path=None):
    ...
    DF_KY_WD = pd.DataFrame(
        list(WordTerm.objects.values(
            'word',
            'term_id',
            'term_id__term',
            'term_id__description'
            ).filter(status=True, commute=True).order_by('word'))
        )
    ...
    try:
        ...
        for future_to in as_completed(future):
            ...
            df_reducer = pd.concat([df_reducer, df_combiner], axis=0)  # trocar pot lista para ganhar performance / exemplo ncbi  # noqa E501
    except Exception as e:
